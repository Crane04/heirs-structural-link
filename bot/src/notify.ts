...
export function buildClaimReadyMessage(claimId: string, totalPayoutNgn: number): string {
  ...
  return (
    ...
    `${process.env.WEB_APP_URL}/claim/${claimId}/report`
  );
}
...
export function buildWelcomeMessage(claimId: string, name = 'there'): string {
  return (
    ...
    `${process.env.WEB_APP_URL}/claim/${claimId}/scan\n\n` +
