...
function severityColor(s: string): string {
  ...
  return map[s] ?? 'text-ink';
}

function SeverityBadge({ severity }: { severity: string }) {
  const color = severityColor(severity);
  return (
    <span className={`text-xs font-bold uppercase tracking-wider px-2 py-0.5 rounded-full bg-ink/10 ${color}`}>
      {severity}
    </span>
  );
}
  ...
  return (
    <div className="bg-surface border border-border/60 rounded-2xl overflow-hidden">
      <button
        ...
        className="w-full flex items-center justify-between px-4 py-4 text-left hover:bg-ink/5 transition-colors"
      >
        <div className="flex items-center gap-3">
          <div className={`w-2.5 h-2.5 rounded-full ${p.fraudFlagged ? 'bg-gold' : 'bg-ink'}`} />
          <div>
            <p className="text-ink font-semibold text-sm capitalize">{p.zone.replace(/_/g, ' ')}</p>
            <p className="text-ink/40 text-xs capitalize">{p.damageType} · {p.dentDepthCm.toFixed(1)}cm depth</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <SeverityBadge severity={p.severity} />
          <span className="text-ink/30 text-lg">{expanded ? '−' : '+'}</span>
        </div>
      </button>
      ...
      {expanded && (
        <div className="px-4 pb-5 flex flex-col gap-3 border-t border-border/40 pt-4">
          <div>
            <p className="text-ink/40 text-xs uppercase tracking-wider mb-1">Visible Damage</p>
            <p className="text-ink/80 text-sm">{p.payoutParts}</p>
          </div>
          <div>
            <p className="text-ink/40 text-xs uppercase tracking-wider mb-1">Predicted Hidden Damage</p>
            <p className="text-ink text-sm">{p.hiddenDamage}</p>
          </div>
          <div>
            <p className="text-ink/40 text-xs uppercase tracking-wider mb-1">Components at Risk</p>
            <p className="text-ink/70 text-sm">{p.componentsAtRisk}</p>
          </div>
          <div className="flex items-center justify-between pt-1">
            <div>
              <p className="text-ink/40 text-xs uppercase tracking-wider">Severity Score</p>
              <p className={`text-lg font-bold ${severityColor(p.severity)}`}>{p.severityScore}/10</p>
            </div>
            <div className="text-right">
              <p className="text-ink/40 text-xs uppercase tracking-wider">AI Confidence</p>
              <p className="text-ink font-bold">{Math.round(p.confidence * 100)}%</p>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="pb-28">
        <div className="flex items-end justify-between flex-wrap gap-6">
          <div>
            ...
            <div className="text-ink/55 text-sm mt-2">
              Claim {claimId?.slice(0, 8).toUpperCase()} • Generated via Structural‑Link AI
            </div>
          </div>
          <span className="hs-chip">
            <span className={`w-2 h-2 rounded-full ${report.fraudFlagged ? 'bg-gold' : 'bg-ink'}`} />
        ...
        <div className="mt-6 grid grid-cols-1 lg:grid-cols-3 gap-6">
          <Card className="p-6">
            <div className="text-[11px] tracking-widest uppercase text-ink/45">Estimated payout</div>
            <div className="mt-3 text-4xl font-bold">{formatNgn(report.totalPayoutNgn)}</div>
            <div className="mt-2 text-xs text-ink/45">Based on Ladipo Market seed pricing</div>
          </Card>
          <Card className="p-6">
            <div className="text-[11px] tracking-widest uppercase text-ink/45">Fraud flagged</div>
          ...
          <Card className="p-6">
            <div className="text-[11px] tracking-widest uppercase text-ink/45">Zones detected</div>
            <div className="mt-3 text-4xl font-bold">{zonesDetected}</div>
            <div className="mt-2 text-xs text-ink/45">Frontal & lateral impact clusters</div>
          </Card>
        </div>
        ...
        <div className="mt-6 grid grid-cols-1 lg:grid-cols-2 gap-6 items-start">
          <Card className="p-6">
            <div className="text-[11px] tracking-widest uppercase text-ink/45 mb-4">Damage map</div>
          ...
          <div>
            <div className="text-[11px] tracking-widest uppercase text-ink/45 mb-4">
        ...
        <div className="mt-6">
          <Card className="p-5 flex items-center justify-between gap-4">
            <div className="text-sm text-ink/55">Claim reference</div>
            <div className="font-mono text-xs text-ink/80">{claimId}</div>
          </Card>
        </div>
      </div>

      {!report.fraudFlagged && (
        <div className="fixed bottom-0 left-0 right-0 z-30 bg-offwhite/90 backdrop-blur border-t border-border">
